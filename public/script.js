...
async function updateTimeSlots() {
  ...
  if (Number(peopleInput.value) > remainingForSelectedTime)
    peopleInput.value = remainingForSelectedTime;

  // Disable input if fully booked
  peopleInput.disabled = remainingForSelectedTime === 0;
  if (remainingForSelectedTime === 0) peopleInput.value = 0;
...
form.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (data.error) {
      ...
      messageBox.textContent = `Booking successful! Your booking number is ${data.bookingId}`;
      form.reset();
      updateTotal();
      await loadSessions();
    }
